...
/**
 * Tool useful to set the air supply generator configuration.
 *
 * @author AdrianSR / 19/10/2021 / 06:03 p. m.
 */
public class BattlefieldSetupToolAirSupply extends BattlefieldSetupToolGUI {
	...
	protected BattlefieldSetupToolAirSupply ( BattlefieldSetupSession session , Player configurator ) {
		...
		if ( result != null ) {
			minimum = result.getConfiguration ( ).getAirSupplyMin ( );
			maximum = result.getConfiguration ( ).getAirSupplyMax ( );
		}
	}
	...
	protected ItemMenu build ( ) {
		final ItemMenu handle = new ItemMenu ( ChatColor.BLACK + "Air Supply" , ItemMenuSize.FOUR_LINE );
		
		// >>>>>>>>>>>>>>>> minimum
		handle.setItem ( 11 , new VoidActionItem (
				ChatColor.GOLD + "Minimum" , UniversalMaterial.LEAD.getItemStack ( ) ,
				"" ,
				ChatColor.GRAY + "Current value: " + ChatColor.GOLD + minimum ,
				"" ,
				ChatColor.GRAY + "This value represents" ,
				ChatColor.GRAY + "the minimum number of air" ,
				ChatColor.GRAY + "supplies that must be dropped" ,
				ChatColor.GRAY + "in the match/arena." ) );
		...
		handle.setItem ( 15 , new VoidActionItem (
				ChatColor.GOLD + "Maximum" , UniversalMaterial.LEAD.getItemStack ( ) ,
				"" ,
				ChatColor.GRAY + "Current value: " + ChatColor.GOLD + maximum ,
				"" ,
				ChatColor.GRAY + "This value represents" ,
				ChatColor.GRAY + "the maximum number of air" ,
				ChatColor.GRAY + "supplies that can be dropped" ,
				ChatColor.GRAY + "in the match/arena." ) );
		...
		handle.setItem ( 35 , new Item (
			...
			public void onClick ( ItemClickAction action ) {
				...
				// setting
				session.setAirSupplyMin ( minimum );
				session.setAirSupplyMax ( maximum );
