 ...
 */
public abstract class BusInstanceBaseTest < C extends BusTest > extends BusInstanceTest < C > {
	...
	protected static class BusDisplacementTask extends BukkitRunnableWrapper {
		...
		protected final BusInstanceBaseTest < ? > bus;
		protected final Vector                    direction;
		protected final ZoneBounds                bounds;
		protected       boolean                   entered_bounds;
		
		public BusDisplacementTask ( BusInstanceBaseTest < ? > bus ) {
			this.bus       = bus;
		...
		public void run ( ) {
			...
			if ( out_bounds && entered_bounds ) {
				System.out.println ( ">>>> BusInstanceBaseTEst: FINISHING" );
				if ( !bus.isFinished ( ) ) {
					bus.finish ( );
				}
	...
	protected BusInstanceBaseTest ( C configuration ) {
