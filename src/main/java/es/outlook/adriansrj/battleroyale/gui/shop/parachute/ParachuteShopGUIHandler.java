package es.outlook.adriansrj.battleroyale.gui.shop.parachute;
 ...
 */
public final class ParachuteShopGUIHandler extends PluginHandler {
	
	public static ParachuteShopGUIHandler getInstance ( ) {
		return getPluginHandler ( ParachuteShopGUIHandler.class );
	}
	 ...
	 */
	public ParachuteShopGUIHandler ( BattleRoyale plugin ) {
		super ( plugin );
	}
	...
	private synchronized ItemMenu build ( org.bukkit.entity.Player player ) {
		BookItemMenu handle = this.handle_map.computeIfAbsent ( player.getUniqueId ( ) , k -> {
			BookItemMenu result = new BookItemMenu (
					EnumShopGUIsConfiguration.PARACHUTE_GUI_TITLE.getAsString ( ) ,
					EnumShopGUIsConfiguration.PARACHUTE_GUI_SIZE.getAsEnum ( ItemMenuSize.class ) );
		...
		// parachutes
		ParachuteRegistry.getInstance ( ).getRegisteredParachutes ( ).stream ( )
				.filter ( parachute -> !CosmeticUtil.isUnlocked ( parachute , player ) )
				.filter ( parachute -> parachute.getPrice ( ) > 0 ) // purchasables only
	...
	private synchronized Item buildParachuteItem ( Parachute parachute ) {
		...
		// parachute must be in the registry
		if ( key != null ) {
			int price = parachute.getPrice ( );
			
			// display text
			String display_text = String.format (
					EnumShopGUIsConfiguration.PARACHUTE_GUI_ITEM_TEXT_FORMAT
			...
			List < String > display_description =
					EnumShopGUIsConfiguration.PARACHUTE_GUI_ITEM_DESCRIPTION_FORMAT.getAsStringList ( );
			...
			return new ActionItem ( display_text , material.getItemStack ( ) , display_description ).addAction ( action -> {
				...
				if ( balance >= price ) {
					ShopConfirmationGUIHandler.getInstance ( ).open ( br_player , ( p , confirm ) -> {
						if ( confirm ) {
							// adding
							data_storage.addCosmetic ( new ParachuteCosmetic ( parachute ) , true );
							// withdrawing balance
							data_storage.balanceWithdraw ( price , true );
							
							// message
							action.getPlayer ( ).sendMessage (
									EnumShopGUIsConfiguration.PARACHUTE_GUI_ITEM_PURCHASED_MESSAGE.getAsString ( ) );
						} else {
							ParachuteShopGUIHandler.this.open ( action.getPlayer ( ) );
						}
					} );
				} else {
					action.getPlayer ( ).sendMessage (
							EnumShopGUIsConfiguration.PARACHUTE_GUI_ITEM_NOT_PURCHASED_MESSAGE.getAsString ( ) );
				}
			} );
		}
		return null;
