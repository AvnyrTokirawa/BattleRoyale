...
public class SchematicGenerator_v13 extends SchematicGeneratorBase {
	...
	public SchematicGenerator_v13 ( File world_folder , EnumDataVersion data_version ) {
		super ( world_folder );
	...
	public SchematicGenerator_v13 ( World world , EnumDataVersion data_version ) {
		super ( world );
		Validate.isTrue ( data_version.getId ( ) >= EnumDataVersion.v1_13.getId ( ) ,
						  "unsupported data version" );
		...
		for ( int y = 0 ; y < height ; y++ ) {
			...
			for ( int z = 0 ; z < depth ; z++ ) {
				...
				for ( int x = 0 ; x < width ; x++ ) {
					...
					try {
						chunk = ( Chunk13 ) chunk_provider.getChunk ( chunk_location );
